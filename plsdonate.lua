...
_G.raised = game:GetService("Players").LocalPlayer.leaderstats.Raised
...
queueonteleport("loadstring(game:HttpGet('https://raw.githubusercontent.com/HMECODES/open-source/main/plsdonate.lua'))()")
local library = loadstring(game:HttpGet("https://raw.githubusercontent.com/HMECODES/customLib/main/plsdonate.lua"))()
...
if isfile("plsdonateupdatedversion.txt") then
	local sl, er = pcall(function()
		getgenv().updatedVersionSettings = httpservice:JSONDecode(readfile('plsdonateupdatedversion.txt'))
	end)
	if er ~= nil then
		task.spawn(function()
...
local sNames = {"textUpdateToggle", "textUpdateDelay", "serverHopToggle", "serverHopDelay", "hexBox", "goalBox", "webhookToggle", "webhookBox", "danceChoice", "thanksMessage", "signToggle", "customBoothText", "signUpdateToggle", "signText", "signHexBox", "autoThanks", "autoBeg", "begMessage", "begDelay", "fpsLimit", "render", "thanksDelay", "vcServer", "censorHop", "autoJump", "autoDie", "deathDelay", "autoJumpWebhooks"}
local sValues = {true, 30, true, 30, "#32CD32", 5, false, "", "Disabled", {"Thank you", "Thanks!", "ty :)", "tysm!"}, false, "WEEKLY GOAL: $C / $G", false, "your text here", "#ffffff", true, false, {"Please donate", "I'm so close to my goal!", "donate to me", "please"}, 300, 60, false, 3, false, false, false, false, 5, false}
...
local donohopToggle = serverHopTab:AddSwitch("Server Hop On Donate", function(bool)
	if updatedVersionSettingsLock then
		return
	end
	getgenv().updatedVersionSettings.donohop = bool
	saveupdatedVersionSettings()
	if bool then
		checkdonos(bool)
	end
	vcToggle:Set(getgenv().updatedVersionSettings.donohop)
end)

function checkdonos(aaa)
    _G.checkdonos = aaa
    while _g.checkdonos do
        if game:GetService("Players").Crystixan.leaderstats.Raised ~= _G.raised then
            serverHop()
        end
        wait(.5)
    end
...
if game:GetService("CoreGui").imgui.Windows.Window.Title.Text == "Loading..." then
	game:GetService("CoreGui").imgui.Windows.Window.Title.Text = "PLS DONATE - Hacky Private"
end
